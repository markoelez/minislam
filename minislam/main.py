#!/usr/bin/env python3
from minislam.visual_odometry import VisualOdometry
...
from minislam.dataset import ImageLoader, VideoLoader, DataLoader
import cv2
import yaml
import os
import argparse
...
def fit_point(img_shape: tuple[float, float], pt: tuple[float, float]) -> tuple[float, float]:
    w, h = img_shape
    x, _, z = pt
...
def main(camera: Camera, data_loader: DataLoader):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',
                        metavar='dataset',
                        type=str,
                        help='The target dataset in config.yaml',
                        default='test')
